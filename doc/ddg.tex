For each constraint not transformed to a oneway constraint auxiliary variables are introduced as invariants and an 
invariant for violation is created. The are created by calling the method \method{createInvariants} of each constraint. 
The algorithm for \class{Linear} was shown in subsection \ref{sub_cons}. These invariants are used to speed up local 
search, that is described in section \ref{sec_local}. \boste{Are they?} \\
...
$x_2$ have an outgoing arc to $y_2$ that has an outgoing arc to $c_2$. A \class{LEQViolation} invariant $y_3$ 
...
SIAM J. Comput., 1(2), 146–160.}. Tarjan's algorithm has a depth first search behavior when exploring vertices and uses 
