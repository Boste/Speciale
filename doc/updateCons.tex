...
\canBeMadeOneway{c,v}\ref{algo_checkoneway} and \makeOneway{c,v}\ref{algo_makeoneway}. The first algorithm check if the 
...
function $f(\vec{y}) \in F$ denoted as $A(f(y))$.  \boste{Maybe call it evalutation functions} Then the coefficent of 
...
\If{$A(c,v) \cdot a > 0$}{\Return{false}}}
\Return{true}\;

 \caption{Test if a constraint $c$ can define a variable $v$ } \label{algo_checkoneway}
\end{algorithm}
\DecMargin{1em}
...
$V(c) = V(c) \backslash \{y\}$\;
\ForEach{$A(c,u)$ \upshape in $A(c)$}{
  $A(c,u) = A(c,u) \cdot \frac{-1}{coef}$
}
\int $b = B(c)$ \;
\If{\relation{c} == Equal}{
